...
一句话：**这个 App 不收集、不上传、不联网**。没有账号，没有统计，没有崩溃上报 —— 连 `INTERNET` 权限都没有。

| 数据 | 存在哪 | 什么时候离开手机 |
| --- | --- | --- |
| 纪念日（标题、日期、备注…） | 应用私有目录里的 SQLite | 只有你自己点「导出备份」或「分享海报」时 |
| 照片封面 | 应用私有目录 `covers/` | 同上 |
| 设置（主题、隐私锁、提醒开关） | 应用私有目录 `settings.json` | 不离开 |

- 卸载 App 等于删掉全部数据，没有云端副本。
- 导出的备份 zip、分享出去的海报，落在哪里由**你自己**选（网盘、相册、聊天窗口），从那一刻起归你管。
- 「隐私锁」是遮罩不是加密，边界见下一节。
...
│   ├── haptics.dart             触感反馈
│   ├── timeline_layout.dart     时光轴的排版（定高行模型，概览带的映射全在这里）
